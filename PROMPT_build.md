...
4. When the tests pass, update @IMPLEMENTATION_PLAN.md, then `git add -A` then `git commit` with a conventional commit message (format: `type: description` where type is feat/fix/docs/refactor/test/chore; use `!` after type for breaking changes like `feat!:` or `fix!:`). After the commit, `git push`.
...
7.       As soon as there are no build or test errors create a git tag with prefix 'ralph-'. If there are no ralph- tags start at ralph-0.0.0 and increment patch by 1 for example ralph-0.0.1 if ralph-0.0.0 does not exist.
...
14.             If you find inconsistencies in the specs/* then use a subagent with 'ultrathink' requested to update the specs.
