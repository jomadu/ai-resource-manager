...
LDFLAGS := -ldflags "-X main.buildVersion=$(VERSION) -X main.buildCommit=$(COMMIT) -X main.buildTimestamp=$(BUILD_TIME) -X github.com/jomadu/ai-rules-manager/internal/version.Version=$(VERSION) -X github.com/jomadu/ai-rules-manager/internal/version.Commit=$(COMMIT) -X github.com/jomadu/ai-rules-manager/internal/version.BuildTime=$(BUILD_TIME) -s -w"
