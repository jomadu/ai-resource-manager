...
func TestNewPackageCache(t *testing.T) {
	...
	cache := NewPackageCache(packagesDir)
	if cache == nil {
...
func TestPackageCache_SetPackageVersion(t *testing.T) {
	...
	err := cache.SetPackageVersion(ctx, packageKey, version, files)
	if err != nil {
...
func TestPackageCache_GetPackageVersion(t *testing.T) {
	...
	err := cache.SetPackageVersion(ctx, packageKey, version, originalFiles)
	if err != nil {
		t.Fatalf("SetPackageVersion() failed: %v", err)
	}
	
	// Then get it back
	retrievedFiles, err := cache.GetPackageVersion(ctx, packageKey, version)
	if err != nil {
...
func TestPackageCache_GetPackageVersion_NotFound(t *testing.T) {
	...
	ctx := context.Background()
	files, err := cache.GetPackageVersion(ctx, packageKey, version)
...
func TestPackageCache_ListPackageVersions(t *testing.T) {
	...
	// List versions
	listedVersions, err := cache.ListPackageVersions(ctx, packageKey)
	if err != nil {
...
func TestPackageCache_ListPackages(t *testing.T) {
	...
	// List packages
	listedPackages, err := cache.ListPackages(ctx)
	if err != nil {
...
func TestPackageCache_RemovePackageVersion(t *testing.T) {
	...
	err = cache.RemovePackageVersion(ctx, packageKey, version)
	if err != nil {
		t.Errorf("RemovePackageVersion() unexpected error: %v", err)
	}
	
	// Verify version is gone
	_, err = cache.GetPackageVersion(ctx, packageKey, version)
	if err == nil {
...
func TestPackageCache_RemovePackage(t *testing.T) {
	...
	err := cache.RemovePackage(ctx, packageKey)
	if err != nil {
...
func TestPackageCache_RemoveOldVersionsByTimestamp(t *testing.T) {
	...
	err := cache.SetPackageVersion(ctx, packageKey, oldVersion, files)
	require.NoError(t, err)
	...
	err = cache.SetPackageVersion(ctx, packageKey, newVersion, files)
	require.NoError(t, err)

	// Remove versions older than 25ms (should remove old version only)
	err = cache.RemoveOldVersions(ctx, 25*time.Millisecond)
	require.NoError(t, err)

	// Check that only new version remains
	versions, err := cache.ListPackageVersions(ctx, packageKey)
	require.NoError(t, err)
...
func TestPackageCache_RemoveUnusedVersionsByAccess(t *testing.T) {
	...
	err := cache.SetPackageVersion(ctx, packageKey, version1, files)
	require.NoError(t, err)
	err = cache.SetPackageVersion(ctx, packageKey, version2, files)
	require.NoError(t, err)
	...
	_, err = cache.GetPackageVersion(ctx, packageKey, version2)
	require.NoError(t, err)

	// Remove versions not accessed in last 25ms (should remove version1 only)
	err = cache.RemoveUnusedVersions(ctx, 25*time.Millisecond)
	require.NoError(t, err)
...
func readPackageMetadata(t *testing.T, tempDir, packageKey string) struct {
	UpdatedAt time.Time `json:"updatedAt"`
} {
	hashedKey, _ := GenerateKey(packageKey)
	metadataPath := filepath.Join(tempDir, hashedKey, "metadata.json")
...
func readVersionMetadata(t *testing.T, tempDir, packageKey string, version core.Version) struct {
	UpdatedAt      time.Time `json:"updatedAt"`
	LastAccessedAt time.Time `json:"lastAccessedAt"`
} {
	hashedKey, _ := GenerateKey(packageKey)
	versionDir := fmt.Sprintf("v%d.%d.%d", version.Major, version.Minor, version.Patch)
	metadataPath := filepath.Join(tempDir, hashedKey, versionDir, "metadata.json")
