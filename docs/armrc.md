...
Cloudsmith registries require API key authentication.

### Configuration

```ini
[registry https://api.cloudsmith.io/owner/repository]
token = your_api_key_here
```

### API Key Format

Cloudsmith API keys typically start with `ckcy_`:
...
```ini
# Production Cloudsmith repository
[registry https://api.cloudsmith.io/myorg/ai-rules]
token = ckcy_prod_token_here

# Development Cloudsmith repository
[registry https://api.cloudsmith.io/myorg/ai-rules-dev]
token = ckcy_dev_token_here
```
